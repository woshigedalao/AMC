...
class R2LTransformer(nn.Module):
    ...
    def forward(self, local_tokens, region_tokens):
        ...
        for attn, ff in self.layers:
            ...
            local_tokens = rearrange(local_tokens, 'b (h p1) (w p2) d -> (b h w) (p1 p2) d', p1=window_size_h,
                                     p2=window_size_w)
            ...
            local_tokens = rearrange(local_tokens, '(b h w) (p1 p2) d -> b (h p1 w p2) d', h=lh // window_size_h,
                                     w=lw // window_size_w, p1=window_size_h)
...
class RegionViT(nn.Module):
    def __init__(
        ...
        for ind, dim, num_layers in zip(range(4), dim, depth):
            ...
            self.layers.append(nn.ModuleList([
                ...
                R2LTransformer(dim, depth=num_layers, window_size=window_size, attn_dropout=attn_dropout,
    ...
    def forward(
        ...
        *_, h, w = x.shape
        # print(h)
        # print(w)
        # print(self.region_patch_size)
        assert divisible_by(h, self.region_patch_size) and divisible_by(w,
                                                                        self.region_patch_size), 'height and width must be divisible by region patch size'
        assert divisible_by(h, self.local_patch_size) and divisible_by(w,
                                                                       self.local_patch_size), 'height and width must be divisible by local patch size'
